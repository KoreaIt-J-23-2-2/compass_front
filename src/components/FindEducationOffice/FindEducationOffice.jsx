...
function FindEducationOffice({ educationOfficeCode }) {
    ...
    const getAcademies = useQuery(
        ...
        },
        {
            ...
            onSuccess: (response) => {
                const newList = response?.data?.academies || [];
                setAcademyData((prev) => {
                    const merged = [...prev.list, ...newList];
                    // ACADEMY_ID 기준 중복 제거
                    const uniqueList = merged.filter(
                        (item, index, self) =>
                            index ===
                            self.findIndex(
                                (t) => t.ACADEMY_ID === item.ACADEMY_ID
                            )
                    );
                    return {
        ...
        <div css={S.SLayout}>
            <div css={S.SHeadContainer}>
                <div css={S.SInput}>
                    <input
                        type="text"
                        onChange={handleAcademyNameInput}
